...
class DummyMoviesAdapter extends DummyMoviesService {
  ...
  override def getMovies: Future[Either[ManagementException, List[DummyMovie]]] = Await.ready(Future {
    try {
      Right(moviesList)
    } catch {
  ...
  override def getMovieById(id: String): Future[Either[ManagementException, Option[DummyMovie]]] = Await.ready(Future {
    try {
  ...
  override def deleteMovie(id: String): Future[Either[ManagementException, Option[DummyMovie]]] = Await.ready(Future {
    try {
